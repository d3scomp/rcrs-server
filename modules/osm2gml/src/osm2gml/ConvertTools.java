...
public final class ConvertTools {
    ...
    */
    ...
    /**
       Compute the nearby-node threshold for an OSMMap.
       @param map The map to look up.
       @return The size of the nearby-node threshold for the map.
    */
    public static double nearbyThreshold(OSMMap map) {
        return sizeOf1Metre(map.getCentreLatitude(), map.getCentreLongitude()) * NEARBY_THRESHOLD_M;
    }
    ...
    */
    public static GMLEdge findLeftTurn(GMLDirectedEdge from, Set<GMLEdge> candidates) {
    ...
    */
    public static GMLEdge findRightTurn(GMLDirectedEdge from, Set<GMLEdge> candidates) {
    ...
    */
    public static GMLEdge findBestTurn(GMLDirectedEdge from, Set<GMLEdge> candidates, boolean preferLeft) {
        GMLEdge mostRight = null;
        GMLEdge mostLeft = null;
        GMLEdge leastRight = null;
        GMLEdge leastLeft = null;
        double mostRightAngle = 0;
        double mostLeftAngle = 0;
        double leastRightAngle = 0;
        double leastLeftAngle = 0;
        Line2D fromLine = gmlDirectedEdgeToLine(from);
        for (GMLEdge next : candidates) {
            if (next.equals(from.getEdge())) {
                continue;
            }
            Line2D nextLine = gmlEdgeToLine(next, from.getEndNode());
    ...
    public static List<ShapeDebugFrame.ShapeInfo> getAllGMLShapes(GMLMap map) {
        List<ShapeDebugFrame.ShapeInfo> allShapes = new ArrayList<ShapeDebugFrame.ShapeInfo>();
        for (GMLNode next : map.getNodes()) {
            allShapes.add(new GMLNodeShapeInfo(next, "Nodes", Constants.BLACK, true));
        }
        for (GMLEdge next : map.getEdges()) {
            allShapes.add(new GMLEdgeShapeInfo(next, "Edges", Constants.BLACK, false));
        }
        for (GMLFace next : map.getFaces()) {
            ...
            case ROAD:
                c = GMLMapViewer.ROAD_COLOUR;
                name = "Roads";
                break;
            case INTERSECTION:
                c = GMLMapViewer.INTERSECTION_COLOUR;
                name = "Intersections";
                break;
            case BUILDING:
                c = GMLMapViewer.BUILDING_COLOUR;
                name = "Buildings";
                break;
            }
            allShapes.add(new GMLFaceShapeInfo(next, name, null, c, false));
        }
        return allShapes;
    }

    public static List<ShapeDebugFrame.ShapeInfo> createGMLDebugShapes(GMLObject... objects) {
        return createGMLDebugShapes(Arrays.asList(objects));
    }

    public static List<ShapeDebugFrame.ShapeInfo> createGMLDebugShapes(Collection<? extends GMLObject> objects) {
        List<ShapeDebugFrame.ShapeInfo> allShapes = new ArrayList<ShapeDebugFrame.ShapeInfo>();
    ...
    */
    ...
    /**
       Find out if a GMLFace is convex.
       @param face The GMLFace to test.
       @return True iff the face is convex.
    */
    public static boolean isConvex(GMLFace face) {
        return isConvex(face.getEdges());
    }
    ...
    /**
       ...
       @return True iff the face is convex.
    */
    public static boolean isConvex(List<GMLDirectedEdge> edges) {
        Iterator<GMLDirectedEdge> it = edges.iterator();
        Line2D first = gmlDirectedEdgeToLine(it.next());
        Line2D a = first;
        Line2D b = gmlDirectedEdgeToLine(it.next());
        boolean rightTurn = GeometryTools2D.isRightTurn(a, b);
        while (it.hasNext()) {
            a = b;
            b = gmlDirectedEdgeToLine(it.next());
    ...
    */
    public static double getAnglesSum(OSMBuilding building, OSMMap map) {
        ...
        double d = getAngle(b, first, second, map);
        if (!Double.isNaN(d)) {
            sum += d;
        }
        return sum;
