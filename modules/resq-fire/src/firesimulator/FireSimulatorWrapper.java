...
public class FireSimulatorWrapper extends StandardSimulator {
	...
	protected void handleUpdate(KSUpdate u) {
		...
		for (EntityID id : u.getChangeSet().getChangedEntities()) {
			...
			if (r == null) {
				...
				} else if (r instanceof WorldInfo && e instanceof rescuecore2.standard.entities.World) {
					WorldInfo w = (WorldInfo) r;
					mapWorldProperties((rescuecore2.standard.entities.World) e, w);
				} else {
	...
	protected void processCommands(KSCommands c, ChangeSet changes) {
		...
		for (Object next : world.getBuildings()) {
			...
			rescuecore2.standard.entities.Building oldB = (rescuecore2.standard.entities.Building) model
					.getEntity(new EntityID(b.getID()));
		...
		for (Object next : world.getFirebrigades()) {
			...
			// if (fb.hasChanged()) {
			rescuecore2.standard.entities.FireBrigade oldFB = (rescuecore2.standard.entities.FireBrigade) model
					.getEntity(new EntityID(fb.getID()));
		...
		// Process wind
		if (world.isWorldInfoDefined()) {
			rescuecore2.standard.entities.World externalWorld = (rescuecore2.standard.entities.World) model
					.getEntity(new EntityID(world.getWorldInfoID()));
			externalWorld.setWindForce(world.getWindForce());
			changes.addChange(externalWorld, externalWorld.getWindForceProperty());
			externalWorld.setWindDirection(world.getWindDirection());
			changes.addChange(externalWorld, externalWorld.getWindDirectionProperty());
		} else {
			Logger.error("WorldInfo in Fire Simulator not defined");
		}
	...
	private void mapWorldProperties(rescuecore2.standard.entities.World oldW, WorldInfo newW) {
		if (oldW.isWindForceDefined()) {
			newW.setWindForce(oldW.getWindForce());
		}
		if(oldW.isWindDirectionDefined()) {
			newW.setWindDirection(oldW.getWindDirection());
		}
	}
